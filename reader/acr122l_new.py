...
class acr122l:
            ...
            if self.rs232_command_done:
                ...
                if self.ser.isOpen():
                    ...
                    if out:
                        ...
                        return self.response_parser(out_int)
                else:
                    return False
            else:
    ...
    def response_parser(self, response):
        print response
        delka = len(response)
        status = response[(delka - 4)]
        check_sum = response[(delka - 2)]
        #check status code
        for s in range(1,3):
            if response[s]>0:
                status_code = False
        ...
        header = []
        for h in range(5,15):
            header.append(response[h])
        
        command = []
        for c in range(15,delka-4):
            command.append(response[c])
        
        c_sum = 0
        for c_c_out in command:
            c_sum ^= c_c_out

        for c_h_out in header:
            c_sum ^= c_h_out
            
            
        if self.rs232_ok == status and status_code:
            if command:
                return command
            else:
                return True
    ...
    def int2asci(self, char):
        return chr(int(hex(char),16))
    ...
    def LCD_back_light(self, state=True):
        ...
        command = [c_class, c_ins, c_p1, c_p2, c_lc]
        return self.rs232_command(command)
    ...
    def LED_control(self, led=False):
        if not self.user_led_control and led:
            ...
            command = [c_class, c_ins, c_p1, c_p2, c_lc]
            self.user_led_control = self.rs232_command(command)
    ...
    def TAG_Polling(self):
        ...
        if ret:
            print str(ret[2]) + ' tags detected'
            
            target_number = ret[3] #Target number 
            sens_res = [ret[4],ret[5]] #SENS_RES  
            sel_res = ret[6] #SEL_RES  
            len_uid = ret[7] #Length of the UID 
            end_uid = 8+len_uid
            uid = []
            for i in range(8, end_uid):
                uid.append(ret[i])
    ...
    def get_sam_version(self, sam=0x01):
        ...
        return self.rs232_command(command,0.1, c_stx, c_etx)
    ...
    def TAG_Authenticate(self, block, key, uid = False, key_type='A'):
        ...
        print self.rs232_command(command)
